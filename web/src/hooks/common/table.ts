...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
  ...
  const {
    ...
    transformer: res => {
      const { records = [], current = 1, size = 10, total = 0 } = res || {};
  ...
  function handleSorterChange(sorter: { columnKey: string; order: 'ascend' | 'descend' | null }) {
    const defaultOrder = '-id'; // 默认排序
    if (!sorter.order) {
      searchParams.orderBy = defaultOrder;
    } else {
...
export function useTableOperate<T extends TableData = TableData>(data: Ref<T[]>, getData: () => Promise<void>) {
  ...
  const checkedRowKeys = ref<bigint[] & string[]>([]);
